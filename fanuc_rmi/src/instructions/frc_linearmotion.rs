...
/// `FRC_LinearMotion` — add a linear motion instruction (Operators Manual
/// §2.4.7 single-group, §2.4.7.1 two-group).
///
...
pub struct FrcLinearMotion {
    ...
    // ---- Optional keys (§2.4.7.1). All omitted from the wire when `None`. ----
    ...
    /// Linear ALIM (single-group per manual §2.4.7).
...
impl FrcLinearMotion {
    /// Single-group (Group 1) linear motion. Signature-compatible with the
    ...
    /// Single-group (Group 1) linear motion (explicit name for [`new`]).
    ///
    /// [`new`]: FrcLinearMotion::new
    pub fn single(
        sequence_id: u32,
        configuration: Configuration,
        position: Position,
    ...
    /// Multi-group linear motion with an explicit [`CartesianGroups`] payload.
    pub fn with_groups(
        sequence_id: u32,
