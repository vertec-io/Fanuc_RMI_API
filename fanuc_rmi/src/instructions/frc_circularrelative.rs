...
/// `FRC_CircularRelative` — add an incremental (relative) circular motion
/// instruction (Operators Manual §2.4.12 single-group, §2.4.12.1 two-group).
///
/// The position payload is carried by [`CircularGroups`], which serializes to
/// the flat single-group form (`Configuration`/`Position`/`ViaConfiguration`/
/// `ViaPosition` at top level) or the wrapped multi-group form (`G1`/`G2` …).
/// For an arm + positioner, build the groups with
/// [`CircularGroups::arm_and_group2`] and set [`coord`] to [`OnOff::ON`] for
/// coordinated motion.
///
/// [`coord`]: FrcCircularRelative::coord
...
pub struct FrcCircularRelative {
    ...
    // ---- Optional keys (§2.4.12.1). All omitted from the wire when `None`. ----
...
impl FrcCircularRelative {
    /// Single-group (Group 1) relative circular motion. Signature-compatible
    /// with the pre-0.6 constructor: builds the flat single-group form, all
    /// options unset.
    pub fn new(
        sequence_id: u32,
    ...
    /// Single-group (Group 1) relative circular motion (explicit name for
    /// [`new`]).
    ///
    /// [`new`]: FrcCircularRelative::new
    pub fn single(
        sequence_id: u32,
        configuration: Configuration,
        position: Position,
    ...
    /// Multi-group relative circular motion with an explicit [`CircularGroups`]
    /// payload.
    pub fn with_groups(
        sequence_id: u32,
    ...
    /// Coordinated arm (Group 1) + Group 2 relative circular motion.
    /// Sets `COORD=ON`.
    pub fn coordinated(
        sequence_id: u32,
        g1: CircGroupBlock,
