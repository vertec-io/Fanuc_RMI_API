...
/// `FRC_JointMotion` — add a joint motion instruction to a Cartesian target
/// (Operators Manual §2.4.9 single-group, §2.4.9.1 two-group).
///
...
/// groups with [`CartesianGroups::arm_and_group2`] and set [`coord`] to
/// [`OnOff::ON`] for coordinated motion.
///
/// [`coord`]: FrcJointMotion::coord
...
pub struct FrcJointMotion {
    ...
    // ---- Optional keys (§2.4.9.1). All omitted from the wire when `None`. ----
    /// Coordinated motion between groups. **Two-group only**: single-group joint
    /// motion with `COORD` returns `RMIT-041` from the controller. `COORD` is
    /// valid only for the two-group coordinated form (§2.4.9.1); use
    /// [`FrcJointMotion::coordinated`], which builds the arm+Group 2 payload and
    /// sets this to [`OnOff::ON`]. Requires the controller's `RMI_MOVE` TP
    /// program to be configured for 2 groups.
...
impl FrcJointMotion {
    ...
    /// Multi-group joint motion with an explicit [`CartesianGroups`] payload.
    pub fn with_groups(
        sequence_id: u32,
